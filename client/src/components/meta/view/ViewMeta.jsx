...
export function Meta({ tag, parent }) {
	...
	return (
		<div className={ `m-2 ml-3 rounded flex flex-col border border-${ color }-200 border-solid shadow` }>
			<div className="flex flex-col">
				<div className="flex flex-row">
					{/* Alias */ }
					<div className={ `basis-1/6 ml-2 p-2` }>{ tag.alias }</div>

					{/* DType */ }
					<div className={ `basis-5/6 p-2 ml-2 font-mono font-bold text-${ color }-${ magnitude }` }>{ tag.dtype }</div>
				</div>
			</div >
			<>
