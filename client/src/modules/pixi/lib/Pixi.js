 ...
 */
export class Pixi {
	constructor ({ width, height, observers = [], container = window } = {}) {
		/**
		...
		this.config = {
			 ...
			 */
			container,
			width: width || container.innerWidth || container.clientWidth,
			height: height || container.innerHeight || container.clientHeight,
	...
	resizeToViewport() {
		return this.resize(
			this.config.container.innerWidth || this.config.container.offsetWidth || this.config.width,
			this.config.container.innerHeight || this.config.container.offsetHeight || this.config.height,
		);
	}
